...
Partial Class tp3 : Inherits Page
    ...
    'Messages d'erreurs pour la validation
    ...
    Sub Page_Load() Handles MyBase.Load
        ...
        'On affiche l'heure de (re)chargement de la page
        lbHeureZoneData.Text = DateTime.Now.ToString()
        lbHeureZoneRapport.Text = DateTime.Now.ToString()
    End Sub
    ...
    Protected Sub btRapport_Click(sender As Object, e As EventArgs) Handles btRapport.Click
        If gvRapport.Visible Then
            gvRapport.Visible = False
            sender.text = "Afficher le rapport"
        Else
            gvRapport.Visible = True
            sender.text = "Cacher le rapport"
        End If
    End Sub
    ...
    Protected Sub CustomServerValidation(source As Object, args As ServerValidateEventArgs)
        ...
        If (args.Value.ToUpper() = "R") Then
            ...
            If (textBoxTraite.ID.ToUpper().Contains("FINAL")) Then
        ...
        If (IsInRange(textBoxTraite.ID, NumberValue, rangeValueMax)) Then
            'On remplace avec le bon séparateur (au besoin), avec 1 décimale et on valide
    ...
    'Lors de la MODIFICATION, SUPPRESSION ou INSERTION dans la base de données, on rafraichie le GridView du Rapport pour le mettre à jour
    Private Sub gvEtudiant_RowDeleted(sender As Object, e As GridViewDeletedEventArgs) Handles gvEtudiant.RowDeleted
        gvRapport.DataBind()
    End Sub
    Private Sub gvEtudiant_RowUpdated(sender As Object, e As GridViewUpdatedEventArgs) Handles gvEtudiant.RowUpdated
        gvRapport.DataBind()
    End Sub
    Private Sub dvInsertion_ItemInserted(sender As Object, e As DetailsViewInsertedEventArgs) Handles dvInsertion.ItemInserted
        gvRapport.DataBind()
    End Sub
